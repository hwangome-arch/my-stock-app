...
def fetch_investor_trend_by_code(code, days=20):
    ...
    try:
        # ── [2026-09] size 상한 확정 ─────────────────────────────────────
        # 여러 size로 테스트해본 결과, 이 비공개 API는 size=100을 넘기면
        # (예: 110/120/125) 그대로 400을 던지는 걸로 확인됐다. 즉 100이 실제
        # 상한이므로, 매번 큰 값부터 재시도하며 낭비하지 않고 그냥 100으로
        # 고정해서 요청한다. (요청한 days가 100보다 작으면 그 값 그대로 사용)
        size = min(max(days + 5, 10), 100)
        url = (
        ...
        if size < max(days + 5, 10):
            debug["size_capped_from"] = max(days + 5, 10)
...
def fetch_screener_data_generator():
    ...
    # ── [진짜 원인 확정] startIdx는 "몇 번째 종목부터"가 아니라 "몇 번째
    # 페이지냐"를 나타내는 값이었다 — 실측으로 확인됨: startIdx=1 응답의 첫
    # 종목("유한양행")이 startIdx=0 응답의 마지막 종목("한미약품") 바로 다음
    # 시가총액 순위였다. 그리고 pageSize=100을 요청해도 서버는 그걸 무시하고
    # 페이지당 고정적으로 약 40개씩만 내려준다. 예전 코드는 "받은 개수만큼
    # start_idx를 전진"시켰는데(0→40→80...), 그러면 그 값이 "40번째 페이지,
    # 80번째 페이지"로 해석되어 존재하지도 않는 페이지를 요청하게 되고, 그
    # 결과 두 번째 요청부터 빈 응답이 와서 스캔이 1페이지(약 40건) 만에 끝나
    # 버렸다 — 계속 "40개"만 나오던 증상의 정확한 원인. 이제는 실제 응답
    # 개수와 무관하게 페이지 번호를 1씩만 증가시킨다.
    all_items = []
    failed_start_idxs = []
    start_idx = 0
    page_num = 0
    ...
    while page_num < _STOCK_API_MAX_PAGES:
        page_num += 1
        progress_pct = 10 + int(min(page_num / 60, 1.0) * 70)  # 정확한 총 페이지 수를 몰라서 대략 표시
        yield f"⚡ 종목 목록 수집 중... ({len(all_items)}개 종목 확보, {page_num}번째 페이지)", progress_pct
        ...
        if items is None:
            # 이 페이지는 끝내 실패 — 하지만 여기서 전체를 포기하지 않고, 이
            # 페이지가 "마지막 페이지"인지 "중간에 막힌 것"인지 알 수 없으므로
            # 일단 다음 페이지로 넘어가서 계속 수집한다(구멍은 아래에서 재시도).
...
_PROBABILITY_FUN_COMPARISONS = [
    ...
    (70, [
        ...
        "😴 아침에 한 번쯤 졸릴 확률",
        "🚇 출퇴근 시간에 사람이 많을 확률",
        "🧦 세탁기에서 양말 한 짝이 사라질 확률",
        "🧼 씻고 나와서 수건을 안 가져온 걸 알 확률",
    ]),
    (60, [
        "📱 하루에 SNS를 한 번 이상 볼 확률",
        "🍜 이번 주에 배달음식을 시킬 확률",
        "🍗 일주일 안에 치킨을 먹을 확률",
        "☕ 하루에 카페를 한 번 갈 확률",
        "🧊 냉장고 열고 먹을 게 없다고 느낄 확률",
        "🍪 과자 한 봉지만 먹으려다 다 먹을 확률",
    ]),
    (50, [
        "🍚 오늘 집에서 밥을 먹을 확률",
        "🎧 하루에 이어폰을 한 번 이상 사용할 확률",
        "☕ 카페에서 아는 메뉴를 주문할 확률",
        "🛒 편의점에서 계획에 없던 걸 살 확률",
        "🔑 열쇠 찾다가 주머니에서 나올 확률",
    ]),
    (40, [
        "🌂 오늘 지나가다 소나기 맞을 확률",
        "🪙 동전 던져 원하는 면이 나올 확률",
        "🍕 피자 마지막 한 조각을 내가 먹을 확률",
        "🚗 주차장에서 내 차 옆자리가 비어있을 확률",
        "🍜 라면 물을 적당히 맞출 확률",
    ]),
    (30, [
        "🚦 신호등이 연속으로 초록불일 확률",
    ...
    (20, [
        ...
        "🎯 찍은 문제가 정답일 확률",
        "📱 배터리가 하루 종일 버틸 확률",
        "🌧️ 우산 없이 나갔는데 비를 피할 확률",
        "🧦 양말을 신었는데 뒤집혀 있을 확률",
    ]),
    (10, [
        "🎴 카드 한 장이 원하는 숫자일 확률",
        "🎁 랜덤 뽑기에서 원하는 상품이 나올 확률",
        "🍕 배달음식이 사진보다 맛있을 확률",
        "🎮 게임 첫판부터 좋은 팀원을 만날 확률",
    ]),
    (5, [
        "🎲 주사위에서 6이 두 번 연속 나올 확률",
        "🪙 동전 5번 중 5번 앞면일 확률",
        "🎯 찍어서 객관식 4문제 모두 맞힐 확률",
        "🛫 비행기에서 유명인을 만날 확률",
        "🌈 무지개를 우연히 발견할 확률",
        "📱 중고거래에서 좋은 물건을 바로 찾을 확률",
    ]),
    (2, [
        "🎰 복권 2등에 당첨될 확률",
        "🃏 포커에서 스트레이트 플러시가 나올 확률",
        "🎯 무작위로 찍어 5문제 모두 맞힐 확률",
        "🐬 해변에서 돌고래를 만날 확률",
        "🌠 별똥별을 우연히 볼 확률",
        "💎 길에서 다이아몬드 주울 확률",
    ]),
...
def draw_fnguide_details(code):
    ...
    if info['name'] != "알 수 없음" or not df_annual.empty:
        ...
        # [2026-09] 네이버 API의 size 상한이 100으로 확인돼, "24주(120거래일)"는
        # 애초에 100거래일 넘게는 못 받아온다. 못 채울 목표치를 라벨에 남겨두는
        # 대신, 실제로 받아올 수 있는 100거래일(≈20주) 기준으로 라벨/일수를 맞춘다.
        _period_days = {"2주 (14일)": 10, "4주 (1개월)": 20, "20주 (100거래일)": 100}[period_choice]
    ...
    if not _reco_df.empty:
        ...
        if _ai_still_loading and ai_grade_filter != "전체보기":
            ...
            if _ai_stalled:
                ...
                # [2026-08-21 수정] 사용자 요청으로 수동 새로고침 버튼 제거.
