...
def fetch_wisereport_data(code):
    ...
    대비한 대체 소스로 검토 중이다.
    🔧 [세션 공유로 수정] 이전엔 토큰(encparam/id) 발급용 GET과 실제 AJAX GET을
    서로 다른 requests.get() 호출로 보내서 세션 쿠키가 전달되지 않았고, 그 결과
    서버가 모든 셀이 동일한 값(예: 7,744)인 디코이(가짜) 데이터를 내려주는 것으로
    확인됨(2026-08). 이제 같은 requests.Session()으로 토큰 발급 → AJAX 호출을
    순서대로 진행해 세션/쿠키가 유지되도록 한다.
    ⚠️ 그래도 JSON이 아니거나 디코이로 의심되면 df_annual/df_quarter는 비워서 반환하고,
    실제 응답 구조를 debug에 남긴다."""
    debug = {"code": code, "step": None}
    df_annual, df_quarter = pd.DataFrame(), pd.DataFrame()
    try:
        session = requests.Session()
        session.headers.update(_WISEREPORT_HEADERS)

        wr_url = f"https://navercomp.wisereport.co.kr/v2/company/c1010001.aspx?cmp_cd={code}"
        res0 = session.get(wr_url, timeout=8)
        res0.encoding = res0.apparent_encoding or 'utf-8'
        debug["token_page_status"] = res0.status_code
        debug["cookies_set"] = list(session.cookies.get_dict().keys())

        encparam, token_id = _get_wisereport_token(code, html_hint=res0.text)
        ...
        def _looks_like_decoy(rows):
            """같은 값이 표 전체에 반복되면(예: 전부 7,744) 디코이 데이터로 간주."""
            try:
                flat = []
                for r in rows[:5]:
                    if isinstance(r, dict):
                        ...
                        flat.extend([str(v) for v in r])
                uniq = set(flat)
                return len(flat) > 3 and len(uniq) <= 1
            except Exception:
                return False
        ...
        for freq_label, freq_typ in [("annual", "Y"), ("quarter", "Q")]:
            try:
                url = "https://navercomp.wisereport.co.kr/v2/company/ajax/cF1001.aspx"
                params = {'cmp_cd': code, 'fin_typ': 0, 'freq_typ': freq_typ, 'encparam': encparam, 'id': token_id}
                res = session.get(url, params=params, headers=ajax_headers, timeout=8)
                res.encoding = res.apparent_encoding or 'utf-8'
                debug[f"{freq_label}_status"] = res.status_code
                debug[f"{freq_label}_content_type"] = res.headers.get('Content-Type')
                try:
                    raw_json = res.json()
                    rows, meta = _parse_wisereport_finsum_json(raw_json)
                    meta["looks_like_decoy"] = _looks_like_decoy(rows) if rows else None
                    ...
                    debug[f"{freq_label}_looks_like_html"] = res.text.strip().startswith('<')
            except Exception as e:
